...
export default function LeadForm() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setStatus("success");
        setForm({ nome: "", email: "", telefone: "", valorDesejado: "" });
      } else {
        setStatus("error");
      }
    } catch {
  ...
  const whatsappMessage = encodeURIComponent(
    `Olá! Meu nome é ${form.nome}. Tenho interesse em um consórcio no valor de R$ ${form.valorDesejado}. Meu telefone: ${form.telefone}`
  );
        ...
        <div className="max-w-lg mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid gap-3 pt-2">
              ...
              <Button
                href={`https://wa.me/5551981179752?text=${whatsappMessage}`}
                variant="secondary"
                size="lg"
            ...
            {status === "error" && (
              <div role="alert" aria-live="assertive" className="p-4 bg-red-500/10 text-red-400 rounded-xl text-sm text-center font-medium">
